...
--
-- THE BODY OF THIS FILE WAS EDITED AFTER IT HAD BEEN APPLIED LOCALLY, and `OR REPLACE` is what lets
-- a fresh application pick the edited body up instead of failing on 42723. THAT IS SAFE ONLY WHILE
-- NO DEPLOYED ENVIRONMENT HAS APPLIED 0061. A local reset rebuilds from zero, so an edited body
-- there is just the body. A migrated environment is different: drizzle-kit records this file by
-- hash in `drizzle.__drizzle_migrations` and skips a file it has already applied, so a later edit
-- never reaches it and raises nothing — the earlier definition stays, silently. 0061 is unmerged as
-- of this edit, so no deployed environment has it and the edit is safe. Once it is applied anywhere
-- that is not rebuilt from zero, this body is frozen and any further change needs its own migration.
