...
export const renderEnumeration = (): string => {
  ...
  return [
    ...
    "",
    table(
      ["prefix", "scope", "object key recorded in", "how a deletion reaches it"],
    ...
    "",
    "**The object keys are on rows the deletion is about to remove.** Every user- and",
    "registration-scoped prefix above takes its exact key from a row inside the CASCADE closure. A",
    ...
    "",
    "The `institution`-scoped prefixes hold institution data, and deleting a user does not touch",
    "them. The `payment-proofs` prefix is the exception in the other direction: its keys live on",
    "rows DEC-0133 forbids deleting, so those objects are exactly addressable and permanently",
