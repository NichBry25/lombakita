...
const main = async () => {
  ...
  record(
    "R2-01",
    "Submission presign returns signed PUT + server key",
    "200 + signed PUT url",
    `${presign.status}`,
  ...
  record(
    "R2-02",
    "Key layout is submissions/{competitionId}/{registrationId}/{uuid}",
    "competition-first prefix",
  ...
  record(
    "R2-03",
    "Real PUT of PDF bytes to R2 succeeds",
    "200",
    `${put1.status}`,
    put1.status === 200,
    put1.text,
  ...
  const rec1 = await apiFetch(
    `/api/v1/competitions/${COMP.inprogress.id}/registrations/${REG.aInprog}/submission`,
    {
      ...
      cookie: cand.cookie,
      headers: hA,
      json: { fileKey: key, fileName: "karya-uji.pdf", fileSizeBytes: pdfBytes().length },
    },
  );
  record(
    "R2-04",
    "Record submission after upload (magic bytes confirmed)",
    "200 version 1",
    `${rec1.status} v${rec1.body?.submission?.version}`,
  ...
  const presign2 = await apiFetch(
    `/api/v1/competitions/${COMP.inprogress.id}/registrations/${REG.aInprog}/submission/upload-url`,
    {
      ...
      cookie: cand.cookie,
      headers: hA,
      json: { fileName: "karya-uji-revisi.pdf" },
    },
  );
  ...
  record(
    ...
    "200 version 2",
    `${rec2.status} v${rec2.body?.submission?.version}`,
  ...
  const recBad = await apiFetch(
    `/api/v1/competitions/${COMP.inprogress.id}/registrations/${REG.aInprog}/submission`,
    {
      ...
      headers: hA,
      json: {
        fileKey: presignBad.body.fileKey,
        fileName: "penipuan.pdf",
  ...
  record(
    "R2-06",
    "PDF extension carrying PNG bytes is REFUSED",
    "422 submission_invalid_file_type",
    `${recBad.status} ${errorCode(recBad)}`,
    refusedWith(recBad, 422, "submission_invalid_file_type"),
    bodySnippet(recBad.body),
  );

  const stillV2 = await apiFetch(
    `/api/v1/competitions/${COMP.inprogress.id}/registrations/${REG.aInprog}/submission`,
    { cookie: cand.cookie },
  );
  record(
    "R2-07",
    "Refused upload did not overwrite the good submission",
    "version 2 intact",
    `v${stillV2.body?.submission?.version}`,
  ...
  record(
    ...
    "Disallowed extension refused at presign",
    "422 submission_invalid_file_type",
    `${presignExe.status} ${errorCode(presignExe)}`,
    refusedWith(presignExe, 422, "submission_invalid_file_type"),
    bodySnippet(presignExe.body),
  );
  ...
  const afterFinal = await apiFetch(
    `/api/v1/competitions/${COMP.inprogress.id}/registrations/${REG.aInprog}/submission`,
    {
      ...
      cookie: cand.cookie,
      headers: hA,
      json: { fileKey: presign2.body.fileKey, fileName: "sesudah-final.pdf", fileSizeBytes: 100 },
    },
  );
  record(
    "R2-10",
    "Replace after finalize refused",
    "422 submission_finalized",
    `${afterFinal.status} ${errorCode(afterFinal)}`,
    refusedWith(afterFinal, 422, "submission_finalized"),
    bodySnippet(afterFinal.body),
  );
  ...
  record(
    "R2-11",
    "Organizer reads the uploaded submission (audited)",
    "200 signed GET url",
    `${orgRead.status}`,
  ...
  if (orgRead.status === 200) {
    ...
    record(
      "R2-12",
      "Presigned GET returns the exact bytes uploaded",
      "PDF magic + same length",
      `${fetched.status} ${buf.length}B`,
  ...
  const create = await apiFetch(
    `/api/v1/institutions/${INST.a.slug}/competitions/${COMP.closing.id}/registrations/${REG.tbC}/document-requests`,
    {
      ...
      cookie: elev.cookie,
      json: {
        title: "Kartu Pelajar (uji otomatis)",
  ...
  record(
    "DOC-01",
    "Organizer raises a document request for one participant",
    "201",
    `${create.status}`,
    create.status === 201,
    bodySnippet(create.body),
  );
  ...
  const dup = await apiFetch(
    `/api/v1/institutions/${INST.a.slug}/competitions/${COMP.closing.id}/registrations/${REG.tbC}/document-requests`,
    {
      method: "POST",
      cookie: elev.cookie,
      json: { title: "Duplikat", dueAt: new Date(Date.now() + 7 * 864e5).toISOString() },
    },
  );
  record(
    "DOC-02",
    "Second open request for the same participant refused",
    "409",
    `${dup.status}`,
    dup.status === 409,
  ...
  const target = (myReqs.body?.requests ?? myReqs.body?.items ?? []).find((r) =>
    String(r.title ?? "").includes("uji otomatis"),
  );
  record(
    "DOC-03",
    "Candidate sees the new request",
    "request present",
  ...
  if (target) {
    ...
    record(
      "DOC-04",
      "Document presign returns signed PUT",
      "200 + signed PUT url",
      `${dPresign.status}`,
    ...
    if (dPresign.status === 200) {
      ...
      const dPut = await putToR2(
        dPresign.body.uploadUrl,
        pngBytes(),
        dPresign.body.contentType ?? "image/png",
      );
      record(
        "DOC-05",
        "Real PUT of PNG document to R2",
        "200",
        `${dPut.status}`,
        dPut.status === 200,
        dPut.text,
      ...
      record(
        "DOC-06",
        "Finalize inspects bytes and writes the row",
        "200",
        `${dFin.status}`,
        dFin.status === 200,
      ...
      const t2 = (afterUpload.body?.requests ?? afterUpload.body?.items ?? []).find(
        (r) => r.id === target.id,
      );
      record(
        "DOC-07",
        "Request moves to 'submitted' after upload",
        "submitted",
        String(t2?.status),
        t2?.status === "submitted",
      );
      ...
      record(
        "DOC-08",
        "Reject with re-upload reopens the request",
        "200",
        `${reject.status}`,
        reject.status === 200,
        bodySnippet(reject.body),
      );

      const afterReject = await apiFetch("/api/v1/me/document-requests", { cookie: candC.cookie });
      const t3 = (afterReject.body?.requests ?? afterReject.body?.items ?? []).find(
        (r) => r.id === target.id,
      );
      record(
        "DOC-09",
        "Reopened request is 'requested' again with the reason retained",
        "requested + note",
        `${t3?.status} / note=${Boolean(t3?.reviewNote)}`,
        t3?.status === "requested",
      );
      ...
      // 404, not 403, and not "either". `loadRequestForCandidate` scopes its query by the caller's
      // own registration, so a request belonging to someone else yields no row and existence
      // collapses into ownership — a foreign id is indistinguishable from a missing one, which is
      // what stops the endpoint enumerating other candidates' requests. Asserting "404 or 403"
      // passes whichever the code answers, so it could not have failed if that collapse were
      // replaced by a 403 that confirms the request exists.
      record(
        "DOC-10",
        "Another candidate cannot upload to someone else's request",
        "404 document_request_not_found",
        `${idor.status} ${errorCode(idor)}`,
        refusedWith(idor, 404, "document_request_not_found"),
        bodySnippet(idor.body),
      );
  ...
  const md = [
    ...
    ...results.map(
      (r) =>
        `| ${r.id} | ${r.name} | ${r.expected} | ${String(r.actual).replaceAll("|", "/")} | ${r.pass ? "✅" : "❌"} | ${String(r.note).replaceAll("|", "/").slice(0, 130)} |`,
    ),
  ].join("\n");
