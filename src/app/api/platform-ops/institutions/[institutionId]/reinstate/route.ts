...
import { reinstateInstitution } from "@/server/moderation/moderation-service";

type RouteParams = { params: Promise<{ institutionId: string }> };

// Reinstate a suspended institution. platform_ops only. Body: { reason: string }.
export async function POST(request: Request, { params }: RouteParams): Promise<Response> {
  try {
    ...
    const result = await reinstateInstitution(session.user.id, institutionId, reason);
