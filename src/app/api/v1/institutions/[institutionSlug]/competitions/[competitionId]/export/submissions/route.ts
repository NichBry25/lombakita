...
import { ExportOwnershipError, exportSubmissionsAsCsv } from "@/server/participants/export-service";
...
export async function GET(_request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    const csv = await exportSubmissionsAsCsv(institutionId, competitionId, db);
    const date = new Date().toISOString().slice(0, 10);

    return new Response(csv, {
      headers: {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="submissions-${competitionId}-${date}.csv"`,
      },
    });
  } catch (error) {
