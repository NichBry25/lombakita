...
const renderQueue = (proofs: OrganiserProofView[]) =>
  render(
    <UIPrimitivesProvider>
      <OrganiserPaymentQueue institutionSlug="seed-academy" competitionId="comp_1" proofs={proofs} />
    </UIPrimitivesProvider>,
  );

describe("OrganiserPaymentQueue", () => {
  it("offers both verdicts on a proof awaiting review", () => {
    // The positive. Every absent-assertion below is worthless without it — they would all pass
  ...
  it("tells the reviewer a rejected payer may still try again, or may not", () => {
    ...
    const { unmount } = renderQueue([
      proof({ status: "rejected", rejectionReason: "Nominal tidak sesuai", resubmissionAllowed: true }),
    ]);
    expect(screen.getByText(/masih dapat mengirim bukti baru/)).toBeTruthy();
    unmount();

    renderQueue([
      proof({ status: "rejected", rejectionReason: "Bukan transfer ke kami", resubmissionAllowed: false }),
    ]);
